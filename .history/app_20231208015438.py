...
class Pixie:
    ...
    def most_similar_colour(colour:tuple[int,int,int], palette:list[tuple[int,int,int]]) -> tuple[int,int,int]:
        ...
        for c in palette:
            ...
            dist = np.sqrt(np.sum( (np.array(colour) - np.array(c))*(np.array(colour) - np.array(c)) ))
            if dist < best_dist:
                best = c
                best_dist = dist
        return best
    @staticmethod
    @lru_cache(maxsize=1024, typed=True)
    def colorSim(rgbColor:tuple[int,int,int], compareColor:tuple[int,int,int]) -> float:
        d = 0
        for c,r in zip(rgbColor, compareColor):
            d += (c - r) ** 2
