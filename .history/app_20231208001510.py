...
class Pixie:
    ...
    def outline( img:Image.Image, outline:Image.Image, pixel_size) -> Image.Image:
        ...
        new_w = w // pixel_size
        res = np.copy(np_img)
        outline_threshold = 200 # threshold for the outline, 0 is min and 255 is max
...
if __name__ == '__main__':
    ...
    inverted = ImageOps.invert(pixellated_edges)
    ...
    sovraimposed_alt = Pixie.sovraimpose(inverted.resize(1000,1000), pixellated_resized)
    sovraimposed_alt.show()
    # blur = Pixie.blur(img, 20)
    # blur.show()
    Pixie.outline(pixellated_resized, edges_resized, 10).show()