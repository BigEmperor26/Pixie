...
if __name__ == '__main__':
    ...
    img_rc = Pixie.recolour(img, 24)
    
    pixellated = Pixie.pixellate(img_rc, 5)
    pixellated.show()
    
    
    small  = Pixie.scale(img, 0.20)
    pixellated = np.array(pixellated)
    pixellated = cv2.GaussianBlur(pixellated, (0,0), 3)
    pixellated = cv2.addWeighted(pixellated, 1.5, pixellated, -0.5, 0)
    pixellated = Image.fromarray(pixellated).show()
    
    # shapre edges
    # img_rc = img_rc.filter(ImageFilter.SHARPEN)
    # cv2 sharpen
    img_rc = np.array(img_rc)
    img_rc = cv2.GaussianBlur(img_rc, (0,0), 3)
    img_rc = cv2.addWeighted(img_rc, 1.5, img_rc, -0.5, 0)
    ...
    # rect = (0,0,np_img.shape[0],np_img.shape[1])
    # cv2.grabCut(np_img,mask,rect,bgdModel,fgdModel,3,cv2.GC_INIT_WITH_RECT)
    ...
    # # edges = Image.fromarray(edges)
    # # edges.show()
    # # final = Pixie.outline_pixels(small, edges,pixel_size=1)
    # # custom_palette = [
    # #   [140, 143, 174],
    # #   [88, 69, 99],
    # #   [62, 33, 55],
    ...
    # # ]
    # # res = tuple(tuple(sub) for sub in custom_palette)
    # # recoloured_custom = Pixie.recolour_palette(small, res, pixel_size=1)
    # recoloured_custom = Pixie.recolour(small, 24)
    # pixelated = Pixie.pixellate_resize(recoloured_custom, 20)
    # pixelated.show()