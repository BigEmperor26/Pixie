...
if __name__ == '__main__':
    ...
    pysharp = small.filter(ImageFilter.EDGE_ENHANCE)
    ...
    k = np.array([[-1,-1,-1], [-1,10,-1], [-1,-1,-1]], dtype=np.int8)/2
    cv_filter = cv2.filter2D(np.array(small), -1, k)
    # import pdb; pdb.set_trace()
    cv_filter = Image.fromarray(cv_filter)
    
    cv2_resize = cv2.resize(np.array(small), (small.width*5, small.height*5), interpolation=cv2.INTER_NEAREST)
