...
class Pixie:
    ...
    #     return outlined_img
    @staticmethod
    def outline( img:Image.Image, outline:Image.Image, color:'white') -> Image.Image:
        """
        follows the edges of the image in order to outline them with 1 pixel of contour
        :param img: Image
        """
...
if __name__ == '__main__':
    ...
    # blur.show()
    recoloured = Pixie.recolour(pixellated, 256)
    recoloured.show()
    
    Pixie.outline(pixellated, pixellated_edges, 'white').show()
    Pixie.outline(pixellated, pixellated_edges, 'black').show()
    Pixie.outline(recoloured, pixellated_edges, 'white').show()
    Pixie.outline(recoloured, pixellated_edges, 'black').show()