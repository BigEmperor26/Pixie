...
class Pixie:
    ...
    def outline( img:Image.Image, outline:Image.Image, pixel_size) -> Image.Image:
        ...
        new_w = w // pixel_size
        res = np.copy(np_img)
        outline_threshold = 220 # threshold for the outline, 0 is min and 255 is max
        outline_mask = np_outline > outline_threshold
        for c in range(np_img.shape[-1]):
            for i in range(new_h):
                for j in range(new_w):
                    # get pixel value as mean of pixel_size x pixel_size square
                    color:bool = np_outline[i*pixel_size:(i+1)*pixel_size,j*pixel_size:(j+1)*pixel_size,c].any()
                    # set pixel value for pixel_size x pixel_size square
                    res[i*pixel_size:(i+1)*pixel_size,j*pixel_size:(j+1)*pixel_size,c] = 255
...
if __name__ == '__main__':
    ...
    # blur.show()
    Pixie.outline(img, edges, 10).show()