...
class Pixie:
    ...
    def recolour_palette( img:Image.Image, palette:list[tuple[int,int,int]], pixel_size) -> Image.Image:
        ...
        :param img: Image
        """
        pal:list = np.array(palette, dtype = np.uint8).flatten().tolist()
        img = img.convert('RGB')
        np_img = np.array(img)
        h,w,c = np_img.shape
        ...
        for i in range(0,h-1, pixel_size):
            for j in range(0,w-1, pixel_size):
                # get pixel value as mean of pixel_size x pixel_size square
                pixel = Pixie.most_similar_colour(tuple(np_img[i,j,:]), tuple(pal))
                # set pixel value for pixel_size x pixel_size square
                new_np_img[i:i+pixel_size,j:j+pixel_size,:] = pixel
        return Image.fromarray(new_np_img)
    @staticmethod
    @lru_cache(maxsize=1024, typed=True)
    def color_similarity(rgb_color:tuple[int,int,int], compare_color:tuple[int,int,int]) -> float:
        d = 0
        for c,r in zip(rgb_color, compare_color):
            d += (c - r) ** 2
        return np.sqrt(d)
    @staticmethod
    @lru_cache(maxsize=1024, typed=True)
    def most_similar_colour(colour:tuple[int,int,int], palette:list[tuple[int,int,int]]) -> tuple[int,int,int]:
        ...
        best = palette[0]
        # euclidean distance
        best_dist = Pixie.color_similarity(colour, best)
        for c in palette:
            # dist = np.linalg.norm(np.array(colour) - np.array(c))
            dist = Pixie.color_similarity(colour, c)
            if dist < best_dist:
                best = c
                best_dist = dist
        return best
...
if __name__ == '__main__':
    ...
    recoloured_custom = Pixie.recolour_palette(pixellated, custom_palette, pixel_size=10)
