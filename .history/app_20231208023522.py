    ...
    def outline_pixels( img:Image.Image, outline:Image.Image, pixel_size) -> Image.Image:
        ...
        for i in range(new_h):
            for j in range(new_w):
                ...
                res[i*pixel_size:(i+1)*pixel_size,j*pixel_size:(j+1)*pixel_size,:]  -= np_outline[i*pixel_size:(i+1)*pixel_size,j*pixel_size:(j+1)*pixel_size].any()
                # set pixel value for pixel_size x pixel_size square
                if color:
...
if __name__ == '__main__':
    ...
    pixellated_edges.show()
    Pixie.outline_pixels(pixellated, pixellated_edges,pixel_size=5).show()
