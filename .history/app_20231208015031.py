...
class Pixie:
    ...
    def most_similar_colour(colour:tuple[int,int,int], palette:list[tuple[int,int,int]]) -> tuple[int,int,int]:
        ...
        best = palette[0]
        # euclidean distance
        best_dist = np.sqrt(np.sum((np.array(colour) - np.array(best))*(np.array(colour) - np.array(best)) ))
        for c in palette:
            # dist = np.linalg.norm(np.array(colour) - np.array(c))
            dist = np.sqrt(np.sum((np.array(colour) - np.array(dist))*(np.array(colour) - np.array(dist)) ))
            if dist < best_dist:
                best = c
                best_dist = dist
        return best
