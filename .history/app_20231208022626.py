...
class Pixie:
    ...
    def outline_pixels( img:Image.Image, outline:Image.Image, pixel_size) -> Image.Image:
        ...
        new_w = w // pixel_size
        res = np.copy(np_img)
        outline_threshold = 100 # threshold for the outline, 0 is min and 255 is max
...
if __name__ == '__main__':
    ...
    blurred = pixellated.filter(ImageFilter.GaussianBlur(radius=5))
    blurred.show()
    pixellated_edges = Pixie.edge_detection(blurred)
    pixellated_edges.show()
    Pixie.outline_pixels(pixellated, pixellated_edges,pixel_size=3).show()
